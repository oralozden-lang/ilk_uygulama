                      ...
                      IconButton(
                        ...
                        onPressed: () async {
                          final tumSubeler = await FirebaseFirestore.instance
                              .collection('subeler')
                              .get();
                          final subeIdleri = tumSubeler.docs
                              .map((d) => d.id)
                              .toList();
                          if (context.mounted) {
                            Navigator.push(
                              ...
                              MaterialPageRoute(
                                builder: (_) => OnHazirlikEkrani(
                                  subeKodu: s.id,
                                  subeler: subeIdleri,
                                  gecmisGunHakki: -1, // Yönetici sınırsız
                                ),
                              ),
                            );
                          }
...
class _OnHazirlikEkraniState extends State<OnHazirlikEkrani>
  ...
  Future<void> _kapanmamisGunuBulVeAc() async {
    // Yönetici ise direkt bugünde aç
  ...
  Widget _tarihSeciciSection() {
    ...
    // İleri: gün kapatılmış ve bugünü geçmemişse
    // Kapanmamış günden ileri geçilemez
    final ileriAktif =
        _gunuKapatildi && !_duzenlemeAcik && _secilenTarih.isBefore(bugun);
  ...
  Widget _transferGoruntule(
    ...
    return Column(
      ...
      children: [
        ...
        // GİDEN — Gönder butonu
        // Gönderildi ama onaylanmadıysa: pasif "Gönderildi" butonu
        // Reddedildiyse: aktif "Tekrar Gönder" butonu
        // Henüz gönderilmediyse: aktif "Gönder" butonu
        // Onaylandıysa: buton yok
        if (kategori == 'GİDEN' && !onaylandi) ...[
          const SizedBox(height: 8),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: (gonderildi && !reddedildi) || _readOnly
                  ? null // Gönderilmiş ama reddedilmemiş → pasif
                  : () {
                      final hedef = t['hedefSube'] as String? ?? '';
        ...
        if (kategori == 'GELEN' && !onaylandi) ...[
          const SizedBox(height: 8),
          SizedBox(
            width: double.infinity,
            child: ElevatedButton.icon(
              onPressed: (gonderildi && !reddedildi) || _readOnly
                  ? null // Bildirilmiş ama reddedilmemiş → pasif
                  : () {
                      final kaynak = t['kaynakSube'] as String? ?? '';
                      final tutar0 = _parseDouble(
                        (t['tutarCtrl'] as TextEditingController).text,
        ...
        // GELEN bildirildi ama onaylanmadı — bilgi satırı
        if (kategori == 'GELEN' && gonderildi && !onaylandi && !reddedildi)
          Padding(
            padding: const EdgeInsets.only(top: 6),
            child: Text(
              'Karşı şubeye bildirildi — yanıt bekleniyor',
        ...
        builder: (context, snapshot) {
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              final nakitDovizRaw =
                  (data['nakitDovizler'] as List?)?.cast<Map>() ?? [];
              String nakitDovizListesiGecmis = '';
              for (final nd in nakitDovizRaw) {
                ...
                if (miktar > 0) {
                  ...
                  final sembol = cins == 'USD'
                      ...
                      ? '£'
                      : cins;
                  if (nakitDovizListesiGecmis.isNotEmpty)
                    nakitDovizListesiGecmis += '  ';
                  nakitDovizListesiGecmis += '$sembol${_binAyracSade(miktar)}';
                }
              }
              ...
              // Döviz kasa özeti (sembolle)
              final dovizListKasa =
                  (data['dovizler'] as List?)?.cast<Map>() ?? [];
              String dovizKasaOzet = '';
              for (final d in dovizListKasa) {
                ...
                if (miktar > 0) {
                  ...
                  final sembol = cins == 'USD'
                      ...
                      ? '£'
                      : cins;
                  if (dovizKasaOzet.isNotEmpty) dovizKasaOzet += ' ';
                  dovizKasaOzet += '$sembol${_binAyracSade(miktar)}';
                }
              }
              // Ana Kasa döviz kalanı
              ...
              for (final entry in akDovizMap.entries) {
                ...
                if (miktar > 0) {
                  ...
                  final sembol = cins == 'USD'
                      ...
                      ? '£'
                      : cins;
                  if (akDovizOzet.isNotEmpty) akDovizOzet += '  ';
                  akDovizOzet += '$sembol${_binAyracSade(miktar)}';
                }
              }
              // Döviz bankaya yatan özeti
              final bankaDovizList =
                  (data['bankaDovizler'] as List?)?.cast<Map>() ?? [];
              String dovizYatanOzet = '';
              for (final d in bankaDovizList) {
                ...
                if (miktar > 0) {
                  ...
                  final sembol = cins == 'USD'
                      ...
                      ? '£'
                      : cins;
                  if (dovizYatanOzet.isNotEmpty) dovizYatanOzet += '  ';
                  dovizYatanOzet += '$sembol${_binAyracSade(miktar)}';
                }
              }
              final tarih = data['tarih'] as String? ?? '';
              ...
              return Card(
                ...
                child: ListTile(
                  ...
                  onTap: () {
                    ...
                    if (!yonetici) {
                      final fark = bugun.difference(tarihDt).inDays;
                        ...
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              widget.gecmisGunHakki == 0
                                  ? 'Geçmiş kayıtları düzenleme yetkiniz yok.'
                                  : '${widget.gecmisGunHakki} günden eski kayıtları düzenleyemezsiniz.',
                            ),
                            backgroundColor: Colors.red,
                          ),
                        );
