...
class ProjeksiyonWidgetState extends State<ProjeksiyonWidget>
  ...
  // Şube önbelleği — her "Göster"de tekrar çekilmez
  Map<String, String> _subeAdlariCache = {};
  bool _subelerYuklendi = false;
  ...
  void initState() {
    super.initState();
    // Şubeler + gider türleri paralel yükle
    Future.wait([_giderTurleriYukle(), _subeleriOnYukle()]);
  }

  Future<void> _subeleriOnYukle() async {
    try {
      ...
      if (mounted) {
        _subeAdlariCache = adlar;
        _subelerYuklendi = true;
      }
    } catch (_) {}
  }
  ...
  Future<void> _yukle() async {
    ...
    try {
      ...
      // Şubeler önbellekte yoksa çek, varsa direkt kullan
      if (!_subelerYuklendi) await _subeleriOnYukle();
      final subeAdlari = Map<String, String>.from(_subeAdlariCache);
      final aktifSubeIdleri = subeAdlari.keys.toList();

      // Tüm şubeler paralel
      final futures = aktifSubeIdleri.map((subeId) async {
  ...
  Widget build(BuildContext context) {
    ...
    for (final v in _subeVeriler) {
      ...
      final tahminGunluk = _parseD(_tahminCtrl[subeId]!.text);
      ...
      child: Column(
        ...
        children: [
          ...
          if (_subeVeriler.isNotEmpty) ...[
                ...
                child: Column(
                  children: [
                    ...
                    Row(
                      children: [
                        Expanded(
                            child: _toplamHucre('Gerçekleşen',
                                toplamGerceklesen, Colors.white)),
                        Expanded(
                            child: _toplamHucre('Tahmini Ay Sonu',
                                toplamTahmini, Colors.white)),
                        Expanded(
                            child: _toplamHucre(
                                'Tahmini Kalan',
                                toplamKalan,
            ...
            Center(
              child: Text('Ay ve yıl seçip "Göster" butonuna basın.',
  ...
  Widget _toplamHucre(String etiket, double deger, Color renk) => Column(
        children: [
          ...
          Text(_fmt(deger),
              style: TextStyle(
                  color: renk, fontSize: 13, fontWeight: FontWeight.bold),
              textAlign: TextAlign.center,
            ...
            child: Container(
              ...
              child: Row(
                children: [
                  ...
                  Icon(detayAcik ? Icons.expand_less : Icons.expand_more,
                      ...
                      Expanded(
                        child: _detayBlok(
                          ...
                          [
                            ...
                            if (gelenTransfer > 0)
                              _detayKalemData(
                                  'Gelen Transfer (+)', gelenTransfer),
                            if (gidenTransfer > 0)
                              _detayKalemData(
                                  'Giden Transfer (−)', -gidenTransfer,
              ...
              children: [
                ...
                if (kalanGun > 0)
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('Günlük ort.:',
                          style: TextStyle(fontSize: 11, color: Colors.grey)),
                      const SizedBox(height: 2),
                      Text(
                        _fmt(gerceklesen /
                            (kalanGun > 0
                                ? (DateTime(_secilenYil, _secilenAy + 1, 0)
                                            .day -
                                        kalanGun)
                                    .clamp(1, 31)
                                : 1)),
                ...
                TextButton.icon(
                  ...
                  label: const Text('Gider Düzenle',
  ...
  Widget _baslikDeger(String etiket, double deger, bool? pozitif) => Column(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          Text(etiket,
  ...
  Widget _detayBlok(
      String baslik, List<Map<String, dynamic>> kalemler, Color baslikRenk) {
    return Container(
  ...
  Map<String, dynamic> _detayKalemData(String ad, double tutar,
          {Color? renk}) =>
